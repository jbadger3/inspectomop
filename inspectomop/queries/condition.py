...
def condition_concept_for_concept_id(concept_id, inspector,return_columns=None):
    """
    Get condition concept for a given condition_concept_id.
...
def condition_concepts_for_source_codes(source_codes, inspector,return_columns=None):
    """
    Get standard condition concepts for source codes.  Ex ICD-9-CM --> SNOMED-CT
    ...
    return_columns: ['source_code', 'source_concept_name',\
...
def source_codes_for_concept_ids(concept_ids, inspector,return_columns=None):
    """
    Get source condition concepts for OMOP concept_ids.  i.e SNOMED-CT --> ICD-9-CM, ICD-10-CM

    Parameters
    ----------
    source_code : list([int])
    ...
    return_columns: ['concept_id','concept_code', 'concept_name',\
            'vocab_id', 'vocab_name',\
...
def pathogen_concept_for_keyword(keyword, inspector,return_columns=None):
    """
    Get pathogen concepts by searching by keyword.
    ...
    -------
    out : inspectomop.Results
    return_columns: ['concept_id', 'concept_name', 'concept_code',\
...
def disease_causing_agents_for_keyword(keyword, inspector,return_columns=None):
    """
    Get disease causing agents by keyword.  The concept class can be any of: Pharmaceutical / biologic product,\
...
def conditions_caused_by_pathogen_or_causative_agent_concept_id(concept_id, inspector,return_columns=None):
    """
    Given a pathogen or other causative agetn concept_id, finds all conditions caused by the agent.
    ...
    -------
    out : inspectomop.Results
    return_columns: ['condition_concept_id', 'condition_name',\
...
def anatomical_site_by_keyword(keyword, inspector,return_columns=None):
    """
    Find an anitomical site given a keyword.  Results of this query are useful for
	...
	WHERE
        ...
        LOWER(C.concept_name) like '%epiglottis%'
	AND
        C.vocabulary_id = V.vocabulary_id AND
	sysdate
	BETWEEN C.valid_start_date AND C.valid_end_date;
...
def condition_concepts_occurring_at_anatomical_site_concept_id(concept_id, inspector,return_columns=None):
    """
    Find conditions that occur at a given anatomical site.  Input concept_id should be a concept of
...
def place_of_service_counts_for_condition_concept_id(condition_concept_id, inspector,return_columns=None):
    ...
    return_columns : ['condition_concept_id', 'condition_concept_id,'place_of_service_concept_id', 'place_of_service', 'place_freq']
    ...
    *****
    Note!  The original query does not function properly in CDM v.5.
    The query CO04: Count In what place of service where condition diagnoses
	SELECT
	...
	FROM (
	    ...
	    FROM (
            ...
            FROM (
                ...
                WHERE condition_concept_id = 31967 AND visit_occurrence_id IS NOT NULL) AS from_cond
        LEFT JOIN ( # j1
	...
	LEFT JOIN (
        ...
        FROM concept) AS place_concept
    ON place_id_count.care_site_id=place_concept.concept_id # This join should actually be on care_site.place_of_service_concept_id
	ORDER BY place_freq;
    """
